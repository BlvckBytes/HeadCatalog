...
public class FakeSlotCommunicator implements IFakeSlotCommunicator {

  private final ConstructorHandle CT_PO_SET_SLOT;
  private final IReflectionHelper reflectionHelper;
  private final ILogger logger;
  private final IWindowOpenWatcher windowOpenWatcher;
  private final Plugin plugin;

  public FakeSlotCommunicator(
    IReflectionHelper reflectionHelper,
    ILogger logger,
    IWindowOpenWatcher windowOpenWatcher,
    Plugin plugin
  ) throws Exception {
    ...
    ClassHandle C_PO_SET_SLOT = reflectionHelper.getClass(RClass.PACKET_O_SET_SLOT);
    ...
    MethodHandle M_AS_NMS_COPY = C_CRAFT_ITEM_STACK.locateMethod()
      .withPublic(true)
      .withStatic(true)
      .withName("asNMSCopy")
      .required();

    // PacketPlayOutSetSlot(int var0, int var1, ItemStack var2) {
  ...
  @Override
  public void setFakeSlot(int slotId, boolean top, ItemStack item, Player player, int delayTicks) {
    try {
      int windowId = 0;
